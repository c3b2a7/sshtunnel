...
latest_tag() {
  ...
  curl -s "$release_url" | grep tag_name | awk '{ print $2 }' | sed 's/[",]//g'
}
...
download_release() {
  ...
  curl -L -s "$releases_url/$version/$archive" -o "$destination"
}
...
install_release() {
  ...
  tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/sshtunnel.XXXXXX") || err "Unable to create temporary directory"
